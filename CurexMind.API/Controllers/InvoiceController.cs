...
using CurexMind.API.Services.Interfaces;
...
namespace CurexMind.API.Controllers
{
    ...
    public class InvoiceController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetInvoiceDetails(Guid invoiceId)
        {
            ...
            if (!User.IsAdmin() && User.GetPatientId() != result.Value.Patient.Id)
                return Forbid();
        ...
        {
            var patientId = User.IsAdmin() ? (Guid?)null : User.GetPatientId();
            var result = await _invoiceService.GeneratePdfExportAsync(invoiceId, patientId);
            if (!result.IsSuccess) return result.ToProblem();
