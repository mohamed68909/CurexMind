...
namespace ClincManagement.API.Controllers
{
    ...
    public class PatientsController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreatePatientAsync([FromForm] PatientRequestDto request, CancellationToken cancellationToken)
        {
            ...
            var result = await _patientService.CreateAsync(createdByUserId, request, cancellationToken);

            return result.IsSuccess
                ? CreatedAtAction(nameof(GetPatientByIdAsync), new { id = result.Value.PatientId }, result.Value)
                : result.ToProblem();
        }
